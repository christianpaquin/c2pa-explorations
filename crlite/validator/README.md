...
Exit status: `0` if the revocation check passed, `1` if it failed (cert revoked at/before signing time, coverage stale, or not-covered under `--uncovered-policy refuse`).

## What it does

1. Loads `artifact.cose`, verifies its ES256 (ECDSA P-256) COSE_Sign1 signature against `publisher.jwk`, and CBOR-decodes the payload.
...
7. Extracts the TSA's signing cert from the COSE_Sign1's `sigTst` (v1) or `sigTst2` (v2) unprotected header — pulls JUMBF directly via `c2pa::jumbf_io::load_jumbf_from_file`, finds the `c2pa.signature` data box, parses the COSE_Sign1 with `coset`, walks into the TimeStampToken (CMS `ContentInfo`/`SignedData` via the `cms` crate), and extracts the first cert in `SignedData.certificates`. Runs the same decision logic with `scope=tsa`.
