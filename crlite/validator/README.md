...
Exit status: `0` if the revocation check passed, `1` if it failed (cert revoked at/before signing time, coverage stale, TSA certificate unavailable, or not-covered under `--uncovered-policy refuse`).
...
6. Applies the v0.2 **coverage-set decision logic** for `(scope=anchors, issuer_ski, serial)`:
   * **Issuer covered and fresh** → authoritative: revoked iff an entry exists and `T_sig ≥ revocation_date`; otherwise good standing.
...
7. Extracts the TSA's signing cert from the COSE_Sign1's `sigTst` (v1) or `sigTst2` (v2) unprotected header — pulls JUMBF directly via `c2pa::jumbf_io::load_jumbf_from_file`, finds the `c2pa.signature` data box, parses the COSE_Sign1 with `coset`, walks into the TimeStampToken (CMS `ContentInfo`/`SignedData` via the `cms` crate), and resolves the signing cert by matching `SignerInfo.sid` against `SignedData.certificates`. Runs the same decision logic with `scope=tsa`; failure to resolve the TSA cert fails the revocation check.
...
See [the design doc's Future work and known gaps](../crlite.md#future-work-and-known-gaps) for the canonical list of PoC gaps (test certificates, production policy choices, etc.).
